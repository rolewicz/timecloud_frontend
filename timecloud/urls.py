...
urlpatterns = patterns('',
   ...
   # (r'^admin/', include(admin.site.urls)),
    ...
    (r'^display/(?P<sensorName>\w+)/(?P<precision>\w+)-(?P<startRow>\w+)-(?P<stopRow>\d+)$', display),
    (r'^display/(?P<sensorName>\w+)$', display),
    (r'^updateTable/', updateTable),
    (r'^visualize/(?P<chartName>\w+)/(?P<sensorName>\w+)/(?P<precision>\w+)-(?P<startRow>\w+)-(?P<stopRow>\d+)$', visualize),
